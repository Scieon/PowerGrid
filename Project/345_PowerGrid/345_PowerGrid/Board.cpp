...
void Board::turnOrder() {
	...
	{   //After first round, it will always go through this one to determine the order
		...
		//DON'T EVEN NEED THIS PART ANYMORE SINCE ADDED FOR ANY X NUMBER OF PLAYERS with function getHighestNumHouse();

		if (vector_player.size() == 2) {
...
{
	//3 if conditions to set the player depending on the current issues
	for (Player* p : vector_player) {
		...
		*/
		if (indexToBuyDefensiveStrategy != -1 && atLeast1House) {//conditino to go inside defensive (if the opponent doesn't have a hosue near you)
			...
			p->executeStrategy(mapOfPlayersCity);
		}
